...
public class FirstMoveCB {
	...
	public static String agentQueryNEW(String[] resource, int[] probability, String name) {
		...
		for (int i = 0; i < probability.length; i++) {
			probability[i] = Math.abs(7 - probability[i]);
		}
		for (int i = 0; i < attrProbability.length; i++) {
			if (i == 2) {
				query += attrProbability[i] + ":" + probability[i];
		...
		// �hnliche F�lle abfragen
		List<Pair<Instance, Similarity>> cases = FirstMoveCases.findSimilarCasesNEW(request);
